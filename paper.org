...
The set of Equations [[eq:cable]], [[eq:synaptic-currents]],
[[eq:space-dep-quant]], [[eq:soma-bound]] and [[eq:end-bound]] together with the
biased Poisson process for event generation (see previous
section[[sec:correlation-generator]]) determine the model.
...
\begin{equation}
 ...
 2^{- \frac{1}{3} \, \lfloor \frac{x}{l \, B} \rfloor}
\end{equation}
...
proximal part length respectively (capital letters index )
...
the means of the synaptic stimulation. For a set of synaptic
stimulation \(\{\nu^e_p, \nu_i^p, \nu_e^d, \nu_i^d, s\}\), the
stationary densities of conductances are given by:
...
wher $\mathcal{D}_e$ and $\mathcal{D}_i$ are the excitatory and
inhibitory synaptic densities respectively (see Figure
1[[fig:model-illustration]]B).

The set of equation governing this mean membrane potential in all
...
& \frac{\partial \mu_v}{\partial x}_{x=0} = R_M \, r_i
\Big( \mu_v(0) - V_0  \Big) \\
...
$v_0^p = \frac{E_L + r_m g_{e0}^p E_e + r_m g_{i0}^p E_i}{ 1 + r_m
g_{e0}^p + r_m g_{i0}^p }$ and $\gamma^p = \frac{C_m \, r_i
\lambda^p}{\tau_S}$

$\lambda^d= \sqrt{\frac{r_m}{r_i (1 + r_m g_{e0}^d + r_m g_{i0}^d)}}$
and $v_0^d = \frac{E_L + r_m g_{e0}^d E_e + r_m g_{i0}^d E_i}{ 1 + r_m
g_{e0}^d + r_m g_{i0}^d }$
...
\frac{\partial \mu_v}{\partial x}_{x=l} & = 0 \\
\frac{\partial \mu_v}{\partial x}|_{x=0} & = \frac{\gamma^p}{\lambda^p}
\Big( \mu_v(0) - V_0  \Big) \\
\mu_v(x \rightarrow l_p^-) & = \mu_v(x \rightarrow l_p^+) \\
\frac{\partial \mu_v}{\partial x}_{x \rightarrow l_p^-} & = \frac{\partial \mu_v}{\partial x}_{x \rightarrow l_p^+} 
\end{split}
