...
* Abstract
  ...
  this property, we investigate here how diverse biophysical features
  in single neuron might contribute to such specific activation. In a
  companion study [[cite:Zerlaut2016]], we reported that, in young mice
  visual cortex, individual neurons differ not only in their
  excitability but also to their sensitivities to the properties of
  ...
  proximal or purely distal. We show that, because of their various
  biophysical properties, individual neurons differentially couple to
  each type of presynaptic activity. The present study therefore
  introduce a new functional impact of biophysical heterogeneity:
  specific coupling to particular types of synaptic activity.
...
of how such specific activation happens are a key question in systems
neuroscience.

[...]

The somato-sensory cortex study of [[citetext:Crochet2011]] showed that,
during active touch, it is the combination of circuit-mediated
integration (the reached level of synaptically driven membrane
potential deflection) and a biophysical feature (the level of membrane
potential threshold for action potential triggering) that determine
the response of a single neuron to sensory input. Therefore
highlighting the role of the interaction between circuit afference and
biophysics for stimulus representation at the single neuron level in
sensory cortex. [[maybe in discussion]]

In the present work, we investigate such interactions in the light of
the variability in the biophysical features found in
[[citetext:Zerlaut2016]], namely that single neurons not only vary in
their excitability (linked to the action potential threshold) but also
in their sensitivity to the properties of the membrane potential
fluctuations.
...
exponent rule [[cite:Rall1962]], the analytical approximation consists in
...
In our companion communication [[cite:Zerlaut2016]], we formulated a two
step procedure to characterize single cell computation



for single cell

formulated [[taken from previous paper, to be adapted]]
...
autocorrelation time $\tau_V$ (see Methods[[sec:autocorrel-def]]). For
example, the excitatory/inhibitory balance controls the mean
depolarization at the soma \mu_V, the mean synaptic bombardment
impacts the standard deviation \sigma_V and the speed of the membrane
potential fluctuations \tau_V. Other effects such as synchrony in the
...
** Diverse coupling to different types of presynaptic activity
...
- Only enables to scan sensory processing in the fluctuation-driven
regime, no temporal coding. Mostly suited to investigate population
dynamics associated to netowrk processing

- no active properties in dendrites, purely passive. could be
  implemented thoug
...
=======
>>>>>>> aa01654075e8b4e63af1495dbed3db5d56ddf04e
...
- different properties for the proximal part vs the distal part.
  ...
  various synaptic properties to obtain the location independence at
  the soma [[cite:Macgee2000]].
...
|                                 | excitatory synapses density | $\mathcal{D}_e^{dist}$ |                                              30.0 | synapses/(100$\mu m^{2}$) |
|                                 | inhibitory synapses density | $\mathcal{D}_i^{dist}$ |                                               8.0 | synapses/(100$\mu m^{2}$) |
|                                 | inhibitory synapses density | $\mathcal{D}_i^{prox}$ |                                              20.0 | synapses/(100$\mu m^{2}$) |
| *Mean morphology*               |                             |                        |                                                   |                           |
|                                 | soma diameter               | D_S                    |                                              15.0 | $\mu m$                   |
|                                 | soma length (mean model)    | L_S                    |                                              10.0 | $\mu m$                   |
|                                 | excitatory synaptic weight  | Q_e^0                  |                                               1.0 | \(nS \)                   |
|                                 | inhibitory synaptic weight  | Q_i^0                  |                                               1.2 | \(nS \)                   |
| *Rescaling rules*               |                             |                        |                                                   |                           |
|                                 | soma diameter               | D_S                    |                                              15.0 | $\mu m$                   |
|                                 | soma length (mean model)    | L_S                    |                                              10.0 | $\mu m$                   |
| for a model of somatic input    | excitatory synaptic weight  | \(Q_e^0\)              | \( Q_e^0 \cdot (400 M \Omega) / R_{in}^{soma}  \) | \(nS \)                   |
| resistance: \(  R_{in}^{soma}\) | inhibitory synaptic weight  | \(Q_i^0\)              | \( Q_i^0 \cdot (400 M \Omega) / R_{in}^{soma}  \) | \(nS \)                   |
...
#+NAME: fig:analytics-accuracy
#+CAPTION: *Accuracy of the analytical approximation for the properties of the membrane potential fluctuations: comparison between numerical simulations and analytical estimates*. *(A)* Morphology corresponding to the mean model, with the 3 points where the membrane potential is recorded. *(B)* Membrane potential fluctuations for the 3 recorded locations. Note the very weal level of correlation of the membrane potential between different branches. *(C)* Properties of the membrane potential fluctuations for different configuration of presynaptic activity: analytical predictions and output from numerical simulations in =NEURON=.
[[./figures/comparison_analytics_and_sim.pdf]]
...
#+CAPTION: *Properties of the membrane potential fluctuations for different types of presynaptic activity: either unbalanced, purely proximal, purely distal, synchronized or non specific (i.e. a mix of all)*. Errorbars represent the variations across neurons (having all a different mmorphology). *(A)* Model variables to achieve varying levels of those different types of activity. *(B)* Membrane potential fluctuations properties and somatic input conductance for the different protocols.
[[./figures/synaptic_integration.pdf]]

** Firing response of neocortical cells
...
[[./figures/firing_response_examples.pdf]]
...
[[./figures/coupling_correlations.pdf]]
...
In this study, the rescaling rule between input resistance at the soma
and the morphological model has led to a rescaling of the 
