...
** Mean membrane potential derivation
<<sec:mean-derivation>>
...
& \gamma^p = \frac{r_i \lambda^p}{R_m \, (1+ G_i^0 RM)}\\
...
with $A$ and $B$ are explicited in the supplementary
section[[sec:mean-coeff]]. The accuracy of this expression can be seen on
Figure [[fig:analytics-accuracy]].


** Equation for the membane potential response to a synaptic event
...
We solve Equation [[eq:psp-equation-reduced]] by using Fourier
...
& \alpha_f^p = \sqrt{1+ 2 i \pi f \tau_m^p} \qquad
 r_f^p = \frac{\tau_m^p}{c_m \, \lambda^p \, (1+ 2 i \pi f \tau_m^p)} \\
& \alpha_f^d = \sqrt{1+ 2 i \pi f \tau_m^d} \qquad
 r_f^d = \frac{\tau_m^d}{c_m \, \lambda^d \, (1+ 2 i \pi f \tau_m^d)} \\
& \qquad \qquad \gamma_f^p =  \gamma^p \, (1+ 2 i \pi f \tau_m^S) \\
\end{split}
\end{equation}

After having performed the Fourier transform, we have used
distribution theory (see [[citetext:Appel2008]] for a comprehensive
textbook) to translate the synaptic input into boundary conditions at
$X_{src}$, physically this corresponds to: 1) the continuity of the
membrane potential and 2) the discontinuity of the current resulting
from the synaptic input.
...
&\left\{
\begin{split}
& A_f(X_{src}) \, \big ( \cosh(r_f^p \, X)+\gamma^p \, \sinh(r_f^p \, X) \big) \\
...
#   dv(X_{src}<=L_p<x) = D(X_{src},f)\, \cosh((x-L)/lbdD)
# if X_{src}>L_p
#   dv(x<=L_p<X_{src}) = A(X_{src},f)\, (\cosh(x/lbdP)+gP\, \sinh(x/lbdP))
#   dv(L_p<x<X_{src}) = B(X_{src},f)\, \cosh((x-L_p)/lbdP)+C(X_{src},f)\, \sinh((x-L_p)/lbdP)
#   dv(L_p<X_{src}<x) = D(X_{src},f)\, \cosh((x-L)/lbdD)
...
** Coefficients for the mean membrane potential solution
<<sec:mean-coeff>>


The first coefficient $A$ is given by:
\begin{equation}
A=\frac{\alpha}{\beta}
\end{equation}
...
The second coefficient $B$ is given by:
\begin{equation}
B=\frac{\gamma}{\delta}
\end{equation}
