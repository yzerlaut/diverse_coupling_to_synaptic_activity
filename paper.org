...
* Abstract
  ...
  activation of subpopulations within the cortical network, and
  specific circuitry is thought to be the primary mechanism underlying
  this feature. Nonetheless, in a companion study [[cite:Zerlaut2016]], we
  found that individual neurons differ in their excitability and
  ...
  (i.e. in the relation from presynaptic quantities to spiking
  probability).
  ...
  impedance in layer V pyramidal neurons of juvenile mice visual
...
value). Biophysical parameters are thus a key properties of stimulus
...
** Heterogeneous firing response imply diverse coupling to different types of presynaptic activity
** Multiplexing of the 
* Discussion
