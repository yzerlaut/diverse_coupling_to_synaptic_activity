...
those new dimensions in the /biophysical specificity/. Here, we aim at
understanding their functional impact. To this purpose, we implemented
various types of presynaptic activity (a form of /stimulus
specificity/) in the fluctuation-driven regime and investigated how
individual neurons would differentially respond those inputs.
...
[[cite:Ecker2010,Renart2010]], the neuronal response can be described as a
spiking probability and the input can be described by presynaptic
firing rates. Thus, we defined /stimulus specificity/ as various types
of presynaptic activity: either unbalanced (to emulate net excitatory
...
For each one of the n=27 cells of our previous study
...
Our study therefore raises the possibility of a multiplexing of the
stimulus features across neurons in the neocortical network supported
by the different firing rate responses of individual neurons in young
mice visual cortex.
...
can elicit depolarizations in a \(sim\) 20mV range (from no
depolarization to a 20mV depolarization) while action potential
threshold spans a \(sim\) 5mV range (see Figure 5D in
[[citetext:Crochet2011]]). Thus there is a factor about 4 between the
/circuit specificity/ (how much one neuron will be depolarized in
...
variabiliy in the local recurrent connectivity and found that this
...
to that of our previous communication).
