...
communication introduced new dimensions in the /biophysical
specificity/: the various sensitivities to the properties of the
fluctuations, and we now aim at understanding their functional impact.
...
specificity/ is null, i.e. all neurons sample 
...
** A simplififed morphological model for dendritic integration
...
\tau_m^0) \). Thanks to the analytical approximation, we can translate
the five model variables \((\nu_e^p, \nu_i^p, \nu_e^d, \nu_i^d, s)\)
into the stationary fluctuations properties \((\mu_V, \sigma_V,
\tau_V)\) so that we finally get the full /input-output/ function
...
section. First, we can see that individual cells have a very different
level of response to the baseline level.
...
variation of the excitability would 
